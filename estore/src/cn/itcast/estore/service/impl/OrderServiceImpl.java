...
public class OrderServiceImpl extends BaseService implements OrderService {
	...
	public boolean generate(String uid, String[] pids, int[] counts) {
		...
		try{
			...
			order.setState(Order.STATE_UNPAY);//新建的订单,状态默认为未付款
			order.setUid(uid);
	...
	public PageBean<Order> findByUidByPage(String uid, int pageNum, int pageSize) {
		...
		try {
			...
			//取得当前用户的订单的总条数
			long count = orderDao.getCountByUid(uid);
			
			//把订单数据和总条数封装到PageBean中
			bean.setData(orders);
	...
	public Order get(String oid) {
		Order order = null;
		try {
			order = orderDao.get(oid);
			//根据OID取得订单项和商品
			List<Map<String,Object>> list = orderDao.findOrderItemAndProductByOid(oid);
			//使用工具方法吧以上ListMap封装成若干个orderItem对象并返回List集合,把集合封装到order对象中
			order.setItems(getOrderItemByOid(list));
		} catch (Exception e) {
			// TODO Auto-generated catch block
